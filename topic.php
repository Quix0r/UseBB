		...
		if ( $functions->auth($topicdata['auth'], 'read', $topicdata['forum_id']) ) {
			...
			while ( $postsdata = $db->fetch_result($result) ) {
				...
				$template->parse('post', 'topic', array(
					...
					'post_content' => $functions->markup($functions->replace_badwords(stripslashes($postsdata['content'])), $postsdata['enable_bbcode'], $postsdata['enable_smilies'], boolval($postsdata['enable_html']), false, $can_post_links),
					'poster_sig' => ( !$functions->get_config('hide_signatures') && !empty($postsdata['signature']) && $postsdata['enable_sig'] ) ? sprintf($template->get_config('sig_format'), $functions->markup($functions->replace_badwords(stripslashes($postsdata['signature'])), $functions->get_config('sig_allow_bbcode'), $functions->get_config('sig_allow_smilies'), false, false, $can_add_profile_links)) : '',
