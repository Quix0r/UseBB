...
class admin_functions {
	 ...
	 */
	public function admin_functions(): void {
			...
			while ( false !== ( $module_name = readdir($handle) ) ) {
				...
				if ( is_array($usebb_module_info) && !array_key_exists($usebb_module_info['short_name'], $this->acp_modules) ) {
					...
					if ( !array_key_exists($usebb_module_info['acp_category'], $this->acp) ) {
						$this->acp[$usebb_module_info['acp_category']] = [];
	 ...
	 */
	public function read_remote_file(string $url): string {
		global $functions;
	 ...
	 */
	public function check_module(string $module_name, string $actual_module_name=''): array|bool {
	 ...
	 */
	public function run_module(string $module_name): void {
		global $lang;
	 ...
	 */
	public function create_acp_menu(string $location): string {
		global $functions, $lang;
	 ...
	 */
	public function create_body(string $location, string $content): void {
		...
		$template->add_breadcrumb($lang['ACP'], ['admin.php']);
		if ( $location != 'index' ) {
			$template->add_breadcrumb($name);
		}

		$template->parse('main', 'admin', [
	 ...
	 */
	public function make_php_string(mixed $variable): string {
	 ...
	 */
	public function set_config(array $settings): void {
		global $functions;
	 ...
	 */
	public function forum_select_box(string $input_name, bool $multiple=true, array $filter_ids=[], string $add=''): string {
		global $db;
		...
		$_POST[$input_name] = ( isset($_POST[$input_name]) && is_array($_POST[$input_name]) ) ? $_POST[$input_name] : [];
	 ...
	 */
	public function delete_forums(string $condition, bool $change_stats=true): void {
		global $functions, $db;
	 ...
	 */
	public function delete_members(string $condition, bool $delete_posts=false, bool $ban_email=false): int {
		global $db, $functions;
		...
		if ( $delete_posts ) {
			...
			//
			$topics = $posts = 0;
			$topic_replies = $topic_firsts = $topic_lasts = $delete_topics = [];
			...
			foreach ( $topic_replies as $topic => $replies ) {
				...
				if ( $replies < 0 ) {
					# Smaller than zero, because 0 = no replies but one post
					$delete_topics[] = $topic;
				}
			}
	 ...
	 */
	public function reload_moderator_perms(): void {
		global $db;
	 ...
	 */
	public function get_cats_array(): array {
		global $db;
	 ...
	 */
	public function get_forums_array(): array {
		global $db;
	 ...
	 */
	public function disable_logout(): void {
		//
	 ...
	 */
	public function set_acp_msg(string $msg): void {
	 ...
	 */
	public function show_acp_msg(): string {
